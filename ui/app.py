...
def main():
    ...
    with st.sidebar:
        ...
        # Parameters
        st.session_state.temperature = st.slider("Temperature", min_value=0.01, max_value=1.0, step=0.01)
        logger.info(f"Selected temperature {st.session_state.temperature}")

        st.session_state.top_p = st.slider("Top-p", min_value=0.01, max_value= 1.0 ,step=0.01)
    ...
    # Here you can render your chat messages
